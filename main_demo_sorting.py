...
def import_excel(x):
    data_file = os.path.join(os.path.dirname(__file__), 'averagespectrafinal.xlsx') 
...
def user_sorting_function(sensors_output):
    ...
    if spectrum.iloc[0] == 0:  # For FTIR, if the first transmittance is 0, it is blank spectra
        ...
        # if decision == {sensor_id : Plastic.LDPE}:
        #     decision = {sensor_id: random.choice([Plastic.LDPE, Plastic.HDPE])}
